...
import { nutricionApi, type NutricionConfigRow } from '@/lib/nutricionApi'
import { cn } from '@/lib/utils'

const CLOUD_URL = (import.meta.env.VITE_NUTRICION_CLOUD_URL as string | undefined)?.trim()

export function NutricionToolbar() {
  const [open, setOpen] = useState(false)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [rows, setRows] = useState<NutricionConfigRow[]>([])
    ...
    void nutricionApi
      ...
      .catch((err: unknown) => {
        if (!cancelled) {
          setRows([])
          setError(
            err instanceof ApiError ? err.message : 'No se pudo cargar el corte',
          )
        }
      })
      .finally(() => {
